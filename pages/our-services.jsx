    ...
    <Layout title="Our Services">
      <Box w="full" py={8}>
        <Container maxW="container.xl">
          <Flex w="full" alignItems="center" justifyContent="space-between" gap={16}>
            <Box w="full">
              <VStack spacing={2} mb={16} alignItems="flex-start">
                <Heading as="h1">
                  Accelerating Growth Through Innovative Digital Solutions:{' '}
                  <i>Unleash Your Potential with Our Web and Mobile App Services</i>
                </Heading>
              </VStack>
              <VStack spacing={8} textAlign="justify">
                <Text>
                  Welcome to <b>Karya Kami Digital</b>, where we turn your digital aspirations into
                  achievements. Our comprehensive suite of web and mobile app development services
                  is meticulously crafted to fast-track the success of digital startups and
                  businesses aiming to elevate their product presence in the digital realm. We fuse
                  cutting-edge technology with imaginative brilliance, delivering solutions that not
                  only meet objectives but redefine them. Join hands with us to sculpt a dynamic
                  digital future.
                </Text>
              </VStack>
            </Box>
        ...
        <Container maxW="container.xl">
          <Flex wrap="wrap" w="full" justifyContent="center" gap={8}>
            {items.map((item, ix) => (
              ...
              >
                <figure>
                  <NextImage src={item.image} alt={item.title} width={512} height={345} />
                  <figcaption className={styles.figureCap}>
                    Photo by{' '}
                    <a href={item.author.authorUrl} target="_blank" rel="noopener noreferrer">
                      {item.author.name}
                    </a>{' '}
                    on{' '}
                    <a href={item.author.vendorUrl} target="_blank" rel="noopener noreferrer">
                      Unsplash
                    </a>
                  </figcaption>
                </figure>
